...
public class TenMinWalk {
    ...
    private static final Map<Character, Integer> directions = new HashMap<>() {{
        put('n', 0);
        put('s', 0);
        put('w', 0);
        put('e', 0);
    ...
    private static final Map<Character, Character> opposite
            = Map.of('n', 's', 's', 'n', 'w', 'e', 'e', 'w');

    public static boolean isValid(char[] walk) {
        if (walk.length != 10) return false;
        directions.replaceAll((k, v) -> v = 0);
        IntStream.range(0, walk.length).mapToObj(i -> walk[i]).forEach(TenMinWalk::accept);
        return directions.entrySet().stream().noneMatch(e -> e.getValue() != 0);
    }

    private static void accept(Character d) {
        directions.merge(d, 1, Integer::sum);
        directions.merge(opposite.get(d), -1, Integer::sum);
    }
}
