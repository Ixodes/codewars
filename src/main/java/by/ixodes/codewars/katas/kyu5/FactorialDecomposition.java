...
public class FactorialDecomposition {
    ...
    public static String decomp(int n) {
        ...
        BigDecimal m = factorial(n);

        for (int i = 2; m.compareTo(BigDecimal.valueOf(i)) >= 0; i += 2) {
            int count = countPrimeFactor(m, BigDecimal.valueOf(i), 0);
            if (count != 0) {
                res.append(i);
                if (count != 1) res.append("^").append(count);
                m = m.divide(BigDecimal.valueOf(Math.pow(i, count)), 1, RoundingMode.HALF_UP);
    ...
    private static int countPrimeFactor(BigDecimal n, BigDecimal prime, int count) {
        return n.remainder(prime).compareTo(BigDecimal.ZERO) != 0
                ? count
                : countPrimeFactor(n.divide(prime, MathContext.UNLIMITED), prime, ++count);
    }
